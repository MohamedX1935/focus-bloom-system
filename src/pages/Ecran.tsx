...
import { DEFAULT_SETTINGS, type ScreenEntry, type AppSettings, type AppCategory, type ScreenAppEntry } from "@/types/app";
...
export default function Ecran() {
  ...
  function recalcTotals(apps: ScreenAppEntry[]) {
    ...
    setData((prev) => ({ ...prev, [today]: { date: today, tempsTotal: total, tempsProductif: productif, tempsNeutre: neutre, tempsDopamine: dopamine } }));
  }
  ...
  function addApp() {
    ...
    const category = classifyApp(name, userClassifications);
    const newEntry: ScreenAppEntry = { appName: name, minutes, category };
    const updated = [...todayApps, newEntry];
    setClassifiedApps((prev) => ({ ...prev, [today]: updated }));
  ...
  function reclassifyApp(idx: number, newCategory: AppCategory) {
    ...
    setUserClassifications((prev) => ({ ...prev, [app.appName.toLowerCase().trim()]: newCategory }));
  ...
  // ─── Score calculation ───
  const entry: ScreenEntry = data[today] || { date: today, tempsTotal: 0, tempsProductif: 0, tempsNeutre: 0, tempsDopamine: 0 };
        ...
        <div className="flex gap-2">
          <div className="flex-1 space-y-1">
            ...
            <Input
              ...
              placeholder="Ex: Instagram, ChatGPT..."
          ...
          <div className="flex items-end">
            <Button onClick={addApp} size="icon" className="h-9 w-9">
              <Plus className="h-4 w-4" />
            </Button>
          </div>
        </div>
        <p className="text-[10px] text-muted-foreground">
          📌 La classification est automatique. Vous pouvez la corriger manuellement.
        </p>
      </motion.div>
      ...
      <div className="space-y-2">
        {todayApps.length === 0 ? (
          <div className="glass-card rounded-xl p-6 text-center text-muted-foreground text-sm">
            Ajoutez vos applications pour calculer le score
          </div>
        ) : (
          todayApps
            ...
            .map((app, idx) => {
              ...
              return (
                ...
                >
                  ...
                  <Button size="icon" variant="ghost" className="h-7 w-7" onClick={() => removeApp(originalIdx)}>
