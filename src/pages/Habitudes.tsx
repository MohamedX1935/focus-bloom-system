...
export default function Habitudes() {
  ...
  const dayHabits: DayHabits = habitsData[today] || {
    date: today,
  ...
  return (
    <div className="space-y-6 max-w-2xl mx-auto">
      ...
      <div className="w-full h-2 rounded-full bg-muted overflow-hidden">
        <motion.div className="h-full rounded-full bg-primary" initial={{ width: 0 }} animate={{ width: `${(doneCount / total) * 100}%` }} transition={{ duration: 0.5 }} />
      </div>
      ...
      <div className="space-y-2">
        {dayHabits.habits.map((habit, i) => (
          ...
          >
            <div className={`w-6 h-6 rounded-md flex items-center justify-center transition-colors ${habit.done ? "bg-primary" : "bg-muted"}`}>
              {habit.done ? <Check className="h-3.5 w-3.5 text-primary-foreground" /> : null}
            </div>
            <span className={`text-sm flex-1 text-left ${habit.done ? "text-muted-foreground line-through" : "text-foreground"}`}>{habit.label}</span>
