...
import { useCalories, useSettings } from "@/hooks/useSupabaseData";
...
export function CalorieTracker() {
  ...
  const entries: CalorieEntry[] = caloriesData[selectedDate] || [];
    ...
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
      ...
      <div className="glass-card rounded-xl p-4 space-y-3">
        ...
        <div className="grid grid-cols-3 gap-2">
          <Input placeholder="Aliment" value={name} onChange={(e) => setName(e.target.value)} className="bg-muted border-border col-span-1" />
          <Input placeholder="Quantité (opt.)" value={quantity} onChange={(e) => setQuantity(e.target.value)} className="bg-muted border-border col-span-1" />
          <div className="flex gap-2">
            <Input type="number" placeholder="kcal" value={calories} onChange={(e) => setCalories(e.target.value)} onKeyDown={(e) => e.key === "Enter" && addEntry()} className="bg-muted border-border flex-1" />
            ...
            {entries.map((entry) => (
              ...
              >
                <div className="flex-1 min-w-0">
                  ...
                  {entry.quantity && <span className="text-[11px] text-muted-foreground">{entry.quantity}</span>}
                </div>
                <div className="flex items-center gap-2">
