...
import { DEFAULT_SETTINGS, EXPENSE_CATEGORIES, type Expense, type AppSettings, type ExpenseCategory } from "@/types/app";
...
export default function Finances() {
  ...
  const [expenses, setExpenses] = useLocalStorage<Expense[]>("discipline-expenses", []);
  ...
  const budgetTotal = Object.values(settings.financeConfig.budgets).reduce((s, v) => s + v, 0);
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }} className="glass-card rounded-xl p-4 space-y-3">
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          <Select value={newCategory} onValueChange={(v) => setNewCategory(v as ExpenseCategory)}>
            <SelectTrigger className="bg-muted border-border w-36">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
          ...
          <Input
            ...
            className="bg-muted border-border flex-1 min-w-[120px]"
          />
          <Button onClick={addExpense} size="icon" className="shrink-0">
          ...
          {byCategory.map((cat) => (
            <div key={cat.value} className="space-y-1">
              ...
              <div className="w-full h-1.5 rounded-full bg-muted overflow-hidden">
                <div
                  className={`h-full rounded-full transition-all ${cat.pct > 100 ? "bg-destructive" : "bg-primary"}`}
